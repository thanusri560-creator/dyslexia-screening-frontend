  ...
  useEffect(() => {
    const loadProgress = async () => {
      ...
      try {
        ...
        const transformedHistory = historyData.map((result: any) => ({
          id: result._id,
          studentId: user.id,
          date: result.createdAt || result.completedAt,
          riskLevel: result.finalRiskLevel,
          score: result.overallScore,
          details: {
            readingTest: result.testScores?.reading || 0,
            phonologicalTasks: result.testScores?.phonological || 0,
            spellingQuiz: result.testScores?.spelling || 0,
          },
        }));

        // Calculate improvement trend
        let improvementTrend = 0;
        if (transformedHistory.length >= 2) {
          const firstScore = transformedHistory[transformedHistory.length - 1].score;
          const lastScore = transformedHistory[0].score;
          improvementTrend = Math.round(((lastScore - firstScore) / firstScore) * 100);
        }

        setProgress({
          studentId: user.id,
          screeningResults: transformedHistory,
          improvementTrend,
        });
