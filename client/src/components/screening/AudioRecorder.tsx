...
export function AudioRecorder({
  ...
  useEffect(() => {
    // Clean up previous audio URL when new audio is recorded
    if (recordedAudio?.url && currentAudioUrlRef.current !== recordedAudio.url) {
      if (currentAudioUrlRef.current) {
        URL.revokeObjectURL(currentAudioUrlRef.current);
      }
      currentAudioUrlRef.current = recordedAudio.url;
    }
  ...
  const togglePlayback = async () => {
    ...
    try {
      ...
      setStatus("Audio playback failed. Please use the controls below.");
      setIsPlaying(false);
    }
  };
